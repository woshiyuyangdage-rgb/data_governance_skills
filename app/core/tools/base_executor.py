"""Shared helper methods for governance tool executors."""
...
class GovernanceToolExecutorBase:
    """Common parsing, tracing, and response helpers for tool mixins."""
    ...
    def _collect_notes_from_agent_result(result: AgentShellResult) -> list[str]:
        ...
        if (
            result.resolved_context is not None
            and result.resolved_context.ambiguity_detected
        ):
            notes.append(
                "Context resolution detected ambiguity and required confirmation."
            )
    ...
    def _resolve_export_profile_name(
        ...
        if (
            workflow_result.progress_snapshot is not None
            or workflow_result.governance_portfolio_summary is not None
        ):
            return "full_governance_portfolio_package"
