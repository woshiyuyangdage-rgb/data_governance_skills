...
def export_workflow_result_to_markdown(
    ...
    if result.skill_outputs:
        for skill_name, payload in result.skill_outputs.items():
            summary = payload.get("summary", "No summary available.") if isinstance(payload, dict) else "No summary available."
            lines.append(f"- **{skill_name}**: {summary}")
    else:
        lines.append("- No skill outputs available.")

    lines.extend(["", "# Diagnosis Issues", ""])
    if result.issues:
    ...
    lines.extend(["", "# Standard Mapping Recommendations", ""])
    ...
    lines.extend(["", "# STG Structure Suggestions", ""])
    ...
    lines.extend(["", "# Quality Rule Recommendations", ""])
    ...
    lines.extend(["", "# Execution-Ready Governance Package", ""])
    ...
    lines.extend(["", "# Governance Readiness Assessment", ""])
    ...
    lines.extend(["", "# Governance Delivery Package", ""])
    ...
    lines.extend(["", "# Batch Processing Summary", ""])
    ...
    lines.extend(["", "# Workbook Import Summary", ""])
    ...
    lines.extend(["", "# Domain Governance Pack", ""])
    if result.domain_pack_match:
        lines.append(f"- Matched pack: `{result.domain_pack_match.matched_pack_name or 'N/A'}`")
    ...
    if result.confirmation_template_match_result:
        ...
        lines.append(f"- Matched template: `{template_match.matched_template_name or 'N/A'}`")
    ...
    if result.confirmation_template_mapping_result:
        ...
        lines.append(f"- Unmapped source columns: {template_mapping.unmapped_source_columns}")
    else:
