...
class QualityRuleRecommendationSkill(BaseSkill):
    ...
    def _build_cross_field_rule(
        ...
        confidence = cls.compute_quality_rule_confidence(confidence_source)
    ...
    def detect_cross_field_patterns(cls, table: TableMeta) -> list[CrossFieldQualityRule]:
        ...
        field_tokens = {
            field.field_name: cls._field_tokens(field.field_name) for field in table.fields
        }
            ...
            for pattern in configured_patterns:
                ...
                if isinstance(trigger_fields, list) and trigger_fields:
                    for trigger in trigger_fields:
                        trigger_tokens = cls._field_tokens(str(trigger))
                        matched = cls._find_field_by_tokens(field_tokens, trigger_tokens)
                        if matched:
        ...
        created = cls._find_field_by_tokens(field_tokens, {"created", "date"})
        updated = cls._find_field_by_tokens(field_tokens, {"updated", "date"})
        ...
        start = cls._find_field_by_tokens(field_tokens, {"start", "date"})
        end = cls._find_field_by_tokens(field_tokens, {"end", "date"})
        if start and end:
            rules.append(
                cls._build_cross_field_rule(
                    table_name=table.table_name,
        ...
        amount = cls._find_field_by_tokens(field_tokens, {"amount"})
        currency = cls._find_field_by_tokens(field_tokens, {"currency"})
        if amount and currency:
            rules.append(
                cls._build_cross_field_rule(
                    ...
                    reason="Amount fields should be interpreted with an associated currency field.",
                    confidence_source="cross_field_pattern",
                )
            )

        status_code = cls._find_field_by_tokens(field_tokens, {"status", "code"})
        status_name = cls._find_field_by_tokens(field_tokens, {"status", "name"})
            ...
            if pair.get("id") and pair.get("name"):
                rules.append(
                    cls._build_cross_field_rule(
                        ...
                        rule_expression=f"{pair['id']} should be consistent with {pair['name']}",
                        severity="low",
                        recommendation_source="weak_hint",
                        match_basis=f"id/name prefix={prefix or 'generic'}",
                        reason="Identifier/name pairs often require reference consistency review.",
                        confidence_source="weak_hint",
                    )
                )

        return cls.deduplicate_cross_field_rules(rules)
    ...
    def detect_domain_rule_candidates(cls, table: TableMeta) -> list[CrossFieldQualityRule]:
        ...
        if not isinstance(domains, dict):
            return []
        table_tokens = cls._table_tokens(table)
        field_tokens = {
            field.field_name: cls._field_tokens(field.field_name) for field in table.fields
        }
        rules: list[CrossFieldQualityRule] = []
        ...
        for domain_name, payload in domains.items():
            ...
            for template in payload.get("rules", []):
                ...
                for token in required_tokens:
                    matched = cls._find_field_by_tokens(field_tokens, {token})
                    if matched:
                        matched_fields.append(matched)
                ...
                rules.append(
                    cls._build_cross_field_rule(
                        ...
                        match_basis=f"domain={domain_name}; required_tokens={','.join(required_tokens)}",
                        reason=(
